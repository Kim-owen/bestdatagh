...
import { createBulkOrder, verifyBulkPhoneNumbers } from "@/lib/bulk.functions";
import { useState, useMemo } from "react";
import { Upload, Plus, Trash2, FileSpreadsheet, Send, ShieldCheck, CheckCircle2, AlertTriangle, RefreshCw } from "lucide-react";
...
type VerificationMap = Record<string, { valid: boolean; network: string; message: string }>;

function BulkPage() {
  const listBundles = useServerFn(listActiveBundles);
  const submit = useServerFn(createBulkOrder);
  const verifyBulkFn = useServerFn(verifyBulkPhoneNumbers);
  ...
  // Bulk Number Verification State
  const [isVerifying, setIsVerifying] = useState(false);
  const [verifications, setVerifications] = useState<VerificationMap>({});
  const [verifySummary, setVerifySummary] = useState<{ total: number; verified: number; invalid: number } | null>(null);
  ...
  const handleBulkVerify = async () => {
    const phoneList = rows.map((r) => r.phone.trim()).filter(Boolean);
    if (phoneList.length === 0) return;

    setIsVerifying(true);
    try {
      const res = await verifyBulkFn({ data: { phones: phoneList } });
      const map: VerificationMap = {};
      res.results.forEach((item) => {
        map[item.phone.trim()] = { valid: item.valid, network: item.network, message: item.message };
      });
      setVerifications(map);
      setVerifySummary({ total: res.total, verified: res.verifiedCount, invalid: res.invalidCount });
    } catch (err: any) {
      setCsvError("Verification error: " + (err.message || "Failed to verify numbers"));
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-[1100px] px-4 sm:px-6 py-10">
        ...
        <p className="mt-2 text-muted-foreground max-w-2xl">Upload a CSV (<code>phone,network,size</code>) or add rows manually. Verify numbers instantly before placing order.</p>
            ...
            <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  disabled={isVerifying || rows.every((r) => !r.phone.trim())}
                  className="inline-flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-3.5 py-2 text-sm font-bold hover:bg-emerald-500/20 disabled:opacity-50 transition-all"
                >
                  <ShieldCheck className={`h-4 w-4 ${isVerifying ? "animate-spin" : ""}`} />
                  <span>{isVerifying ? "Verifying..." : "Bulk Verify Numbers"}</span>
                </button>
              ...
              {verifySummary && (
                <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/30 p-3 text-xs font-semibold text-emerald-600 dark:text-emerald-400 flex items-center justify-between">
                  <span className="flex items-center gap-2">
                    <CheckCircle2 className="h-4 w-4" /> Verified {verifySummary.verified} of {verifySummary.total} phone numbers ({Math.round((verifySummary.verified / verifySummary.total) * 100)}% valid).
                  </span>
                  {verifySummary.invalid > 0 && (
                    <span className="text-amber-500 font-bold flex items-center gap-1">
                      <AlertTriangle className="h-3.5 w-3.5" /> {verifySummary.invalid} invalid number(s) detected
                    </span>
                  )}
                </div>
              )}
            ...
            <div className="rounded-2xl border border-border bg-card overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left"><tr>
                  <th className="p-3 w-10">#</th><th className="p-3">Phone</th><th className="p-3">Status</th><th className="p-3">Bundle</th><th className="p-3"></th>
                </tr></thead>
                <tbody>
                  {rows.map((r, i) => {
                    const clean = r.phone.trim();
                    const v = verifications[clean];
                    return (
                          ...
                          {v ? (
                            v.valid ? (
                              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold ${
                                v.network === "MTN" ? "bg-amber-500/10 text-amber-500 border border-amber-500/30" :
                                v.network === "Telecel" ? "bg-sky-500/10 text-sky-500 border border-sky-500/30" :
                                "bg-emerald-500/10 text-emerald-500 border border-emerald-500/30"
                              }`}>
                                <CheckCircle2 className="h-3 w-3" /> {v.network}
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/30 text-[11px] font-bold">
              ...
              <div className="p-3 border-t border-border flex items-center justify-between">
                ...
                {rows.length > 1 && (
                  ...
                  >
                    <RefreshCw className={`h-3.5 w-3.5 ${isVerifying ? "animate-spin" : ""}`} /> Verify All Rows
                  </button>
                )}
              </div>
            </div>
